...
void test_simple(int size) {
    ...
    std::cout << "result0: " << std::setprecision(std::numeric_limits<double>::digits10 + 2) << sum << ", ";
    printf("Speed: %lf MRSQRT/s\n", (size * 8) * (1e-6) / timer.getTime());
}
...
void test_avxd1(int size) {
    ...
    for (unsigned int j = 0; j < size; j++) {
        ...
        nr = _mm256_mul_pd(_mm256_mul_pd(half, nr), _mm256_sub_pd(three, muls)); // newton iteration
        r0 = _mm256_add_pd(r0, nr);
        // 4,5,6,7
        ...
        nr = _mm256_mul_pd(_mm256_mul_pd(half, nr), _mm256_sub_pd(three, muls)); // newton iteration
        r1 = _mm256_add_pd(r1, nr);
    }
    _mm256_store_pd(result, r0);
    _mm256_store_pd(result + 4, r1);

    timer.stop("avx rsqrt double NWTN 1 complete, ");
    timer.dump();
    double sum = 0;
    for (int i = 0; i < 8; i++) {
        sum += result[i];
...
void test_simplef(int size) {
    ...
    std::cout << "result0: " << std::setprecision(std::numeric_limits<float>::digits10 + 2) << sum << ", ";
    printf("Speed: %lf MRSQRT/s\n", (size * 8) * (1e-6) / timer.getTime());
}
